...
$('.submit-mpesa').on('click', function(){
	...
	//attachment docs
	$('#empIdCard').on('change', function(){
		$('.empId-li i').addClass('zmdi-check-all').addClass('text-green');
		$('.empId-li i').removeClass('zmdi-circle').removeClass('text-red');
	});


	$('#natIdCard').on('change', function(){
		$('.natId-li i').addClass('zmdi-check-all').addClass('text-green');
		$('.natId-li i').removeClass('zmdi-circle').removeClass('text-red');
	});


	// some datatables filtering and groupings
	...
	var table = $('#exampleTable').DataTable({
		...
		if (last !== group) {
		$(rows).eq(i).before('<tr class="group port_table_name bg-black text-white"><td colspan="10">' + group + '</td></tr>');
		last = group;
		}
		});
		}
	...
	$('#exampleTable tbody').on('click', 'tr.group', function() {
	...
	$('.checkboxes-container input').on('change', function(){
		
		var allCheckboxes=$(this).hasClass('all-checkboxes');
		if(allCheckboxes==false){
			checkBoxUnselector();
		}
	});
	...
	$(".time-only").flatpickr({
	  ...
	  $('.elections .card-header i').on('click', function(){
		  $(this).toggleClass('rotate180').toggleClass('text-info');
		  var header= $(this).parent().parent().parent().parent().parent();
			 header.siblings('.card-body').slideToggle();
			 header.toggleClass("orange-border");
			 $(this).parent().siblings('h4').toggleClass('text-info')
	...
	// 	  var test = flatpickr(".time-only", taskFlatpickrConfig);
	// 	var test2 = flatpickr("#tes2", taskFlatpickrConfig);
	//   })

	//   $('#electionDate').on('show.bs.modal', function (e) {
	// 	var taskFlatpickrConfig = {
	// 		enableTime: true,
	// 		altInput: true,
	// 		altFormat: "m-d-Y h:i K"
	...
	//   });
	  ...
	  flatpickr('.time-only',{ 'static': true });
